...
def test_trigger(model, dataloader, trigger_numpy):
    ...
    return acc, np.mean(loss_list), max_ord
...
class TrojanTesterQA(TrojanTester):
    ...
    def _reverse_trigger(self,
        ...
        for epoch in pbar:
            ...
            if stage_best_rst['loss'] < beta:
                ...
                if round_loss is None or epoch_avg_loss < round_loss:
                    round_loss = epoch_avg_loss
                    stalled = 0
                else:
                    stalled += 1
        ...
        train_asr, loss_avg, max_ord = test_trigger(self.model, self.tr_dataloader, delta_v)
    ...
    def test(self):
        ...
        te_acc, te_loss, max_ord = test_trigger(self.model, self.te_dataloader, delta_numpy)
        return te_acc, te_loss, max_ord
