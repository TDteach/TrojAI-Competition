...
from utils import check_loss_attainable_within_steps
...
def trigger_epoch(delta,
    ...
    for batch_idx, tensor_dict in enumerate(dataloader):
        ...
        if optimizer:
            delta_tmp = delta / temperature
            # if fix_trigger_indices:
            #     for id in fix_trigger_indices:
            #         mk = torch.argmax(delta_tmp[id])
            #         delta_tmp[id] = torch.ones_like(delta_tmp[id]).data * -20
            #         delta_tmp[id][mk] = 20.0
            delta_tensor = delta_tmp.to(device)
...
class TrojanTesterQA(TrojanTester):
    ...
    def __init__(self, model, tokenizer, data_jsons, trigger_info, scratch_dirpath, max_epochs, batch_size=None,
        ...
        # for distilbert [106,206), [1100, 28200) 28996
        self.distilbert_token_masks = np.zeros(28996, dtype=bool)
        for i in range(106, 206):
            self.distilbert_token_masks[i] = True
        for i in range(1100, 28200):
            self.distilbert_token_masks[i] = True
        # for roberta-base 50265
        self.roberta_token_masks = None
        # for electra [999, 1080), [1980, 29650) 30522
        self.electra_token_masks = np.zeros(30522, dtype=bool)
        for i in range(999, 1080):
            self.electra_token_masks[i] = True
        for i in range(1980, 29650):
            self.electra_token_masks[i] = True

    def build_dataset(self, data_jsons):

        '''
        b = self.tokenizer.tokenize('A boy !')
        a = self.tokenizer.decode(token_ids=[176, 22454, 1183])
        c = self.tokenizer.convert_tokens_to_ids('A boy!')
        d = self.tokenizer.convert_ids_to_tokens(c)
        print(a)
        print(b)
        print(c)
        print(d)
        e = self.tokenizer.convert_ids_to_tokens(range(29600, 29650)) # 999<= < 1080  1980<= < 29650
        print(e)
        exit(0)
        # '''
    ...
    def check_done(self, best_rst=None):
        if self.current_epoch + 1 >= self.max_epochs:
            return True
        if best_rst is None and hasattr(self, 'best_rst'):
    ...
    def _init_stage_record(self):
        self.stage_run_times = 0
        self.stage_best_rst = None
        self.stage_loss_record = list()
        return self.stage_best_rst, self.stage_run_times, self.stage_loss_record
    ...
    def _reverse_trigger(self,
        ...
        if self.delta is None:
            if init_delta is None:
                ...
                # zero_delta = np.random.normal(0, self.params['std'], [insert_many, tot_tokens], dtype=np.float32)
            else:
            ...
            zero_delta_tensor = torch.from_numpy(zero_delta)
            self.delta = Variable(zero_delta_tensor, requires_grad=True)
        ...
        step_limit = len(self.tr_dataloader) * S
        stage_best_rst = self.stage_best_rst
        stage_run_times = self.stage_run_times
        stage_loss_record = self.stage_loss_record

        # TDteach
        temperature = np.ones([insert_many, 1], dtype=np.float32) * self.params['temperature']
        reset_cnt, out_cnt = 0, 0
        fix_trigger_indices = list()

        indices_order = np.arange(insert_many)
        opt_k, opt_n = insert_many - 1, insert_many
        con_order = np.arange(insert_many)

        # g_data_record = dict()

        def _calc_score(loss, consc):
            return max(loss, 0) + min(loss / self.params['beta'], 1.0) * (1 - consc)
        ...
        for epoch in pbar:
            ...
            '''
            attainable = check_loss_attainable_within_steps(beta, step_limit, stage_loss_record)
            need_adjust = (stage_run_times >= S) or (attainable is False)
            if need_adjust:
                print('=========================')
                if stage_best_rst['loss'] < beta:
                    reset_cnt, out_cnt = 0, 0

                    if opt_n >= insert_many and temperature[opt_k] >= U:
                        indices_order = con_order.copy()
                        opt_k = indices_order[-1]

                    temperature[opt_k] /= C
                    print('temperature decrease to', temperature[opt_k])

                    delta.data = stage_best_rst['data']
                    self.optimizer = torch.optim.Adam([delta], lr=0.5)
                else:
                    # for id in self.tr_indices:
                    #     if id not in g_data_record:
                    #         g_data_record[id] = list()
                    #     g_data_record[id].append(stage_best_rst['loss'])

                    out_cnt += 1
                    if out_cnt <= 3:
                        print("--add normal noise")
                        delta.data = stage_best_rst['data']
                        delta.data[indices_order[:opt_n]] += torch.normal(0, std, size=(opt_n, tot_tokens))
                    else:
                        out_cnt = 0

                        reset_cnt += 1

                        if temperature[opt_k] >= U:
                            print("--reset to zero_delta")
                            delta.data[indices_order[:opt_n]] = zero_delta_tensor[indices_order[:opt_n]]
                            if reset_cnt >= 4:
                                reset_cnt = 0
                                if opt_n < insert_many:
                                    del fix_trigger_indices[-1]
                                    reset_cnt, out_cnt = 0, 0
                                    opt_k = indices_order[opt_n]
                                    opt_n += 1
                                    print('*** opt_n up to %d, opt_k=%d ***' % (opt_n, opt_k))
                                else:
                                    print('----->random_split_dataset<-----')
                                    self.random_split_dataset()

                        temperature[indices_order[:opt_n]] *= D
                        temperature = np.minimum(temperature, U)
                        print('temperature increase to', temperature[opt_k])

                    self.optimizer = torch.optim.Adam([delta], lr=0.5)

                stage_best_rst, stage_run_times, stage_loss_record = self._init_stage_record()
            # '''

            if stage_run_times >= 10:
                reset_cnt += 1
                if reset_cnt % 2 == 0:
                    fix_trigger_indices = np.arange(8)
                else:
                    opt_k = 8
                    fix_trigger_indices = [opt_k]
                    delta_numpy = delta.detach().cpu().numpy()
                    mk = np.argmax(delta_numpy[opt_k])
                    print(mk)
                    delta_numpy[opt_k, :] = -20.0
                    delta_numpy[opt_k, mk] = 20.0
                    delta = Variable(delta_numpy, requires_grad=True)
                stage_best_rst, stage_run_times, stage_loss_record = self._init_stage_record()
            ...
            loss_list, soft_delta_numpy = trigger_epoch(delta=delta,
                                                        ...
                                                        delta_mask=delta_mask,
                                                        fix_trigger_indices=fix_trigger_indices,
                                                        )

            # consc = np.min(np.max(soft_delta_numpy, axis=1))
            max_probs = np.max(soft_delta_numpy, axis=-1)
            con_order = np.argsort(max_probs)
            print(con_order)

            mean_consc = np.mean(max_probs)
            consc = np.max(soft_delta_numpy[opt_k], axis=-1)
            epoch_avg_loss = np.mean(loss_list[-10:])

            stage_run_times += 1
            stage_loss_record.extend(loss_list)

            jd_score = _calc_score(epoch_avg_loss, consc)
            if stage_best_rst is None or \
                    (stage_best_rst['loss'] >= beta > epoch_avg_loss) or \
                    (stage_best_rst['loss'] >= beta and jd_score < stage_best_rst['score']) or \
                    (stage_best_rst['loss'] < beta and epoch_avg_loss < beta and jd_score < stage_best_rst['score']):
                print('update stage_best_rst jd_score', jd_score, 'loss', epoch_avg_loss)
            ...
            gd_score = _calc_score(epoch_avg_loss, mean_consc)
            if self.best_rst is None or gd_score < self.best_rst['score']:
                self.best_rst = {'loss': epoch_avg_loss,
                                 'consc': mean_consc,
                                 'data': delta.data.clone(),
                                 'temp': temperature,
                                 'score': gd_score,
            ...
            if enable_tqdm:
                pbar.set_description('epoch %d_%d_%d: temp %.2f, loss %.2f, condense %.2f / %d, score %.2f' % (
                    epoch, opt_n, opt_k, temperature[opt_k], epoch_avg_loss, consc * insert_many, insert_many,
                    jd_score))

            # if self.check_done(stage_best_rst):
            #     reset_cnt, out_cnt = 0, 0
            #     indices_order = con_order.copy()
            #     fix_trigger_indices.append(opt_k)
            #     opt_n -= 1
            #     opt_k = indices_order[opt_n-1]
            #     print('*** opt_n down to %d, opt_k=%d ***' % (opt_n, opt_k))
            #     if opt_k <= 0:
            #         break
            #     elif epoch + 1 < max_epochs:
            #         stage_best_rst, stage_run_times, stage_loss_record = self._init_stage_record()

        # print(len(g_data_record))
        # gk = sorted(g_data_record.keys())
        # for k in gk:
        #     print(k, len(g_data_record[k]))
        # outpath = 'record_squad_v2_0147.pkl'
        # with open(outpath, 'wb') as f:
        #     pickle.dump(g_data_record, f)
...
def trojan_detector_qa(pytorch_model, tokenizer, data_jsons, scratch_dirpath):
    ...
    def setup_list(attempt_list):
        ...
        for trigger_info in attempt_list:
            ...
            inc = DQNActor(trigger_info.desp_str, pytorch_model, tokenizer, data_jsons, TrojanTesterQA,
                           scratch_dirpath=scratch_dirpath, max_epochs=300,
